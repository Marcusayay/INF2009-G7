...
subprocess.run(["v4l2-ctl", "-d", "/dev/video0", "--set-ctrl=exposure_time_absolute=25"], check=False)  # very short
...
cap = cv2.VideoCapture("/dev/video_tracking")
...
try:
    while True:
        cap.grab()  # flush stale frames from kernel V4L2 buffer
